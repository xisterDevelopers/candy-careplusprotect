					...
					<div class="product-header-info">
						...
						<div class="item <?php echo $indexVariant == 0 ? 'active' : ''; ?>" data-variant="<?= strtolower(get_sub_field('variant_title')) ?>">
							<div class="ctn-carousel-img">
								...
								<?php $img = get_field('category_image_background','category_'.$slaveCategory->term_id) ? get_field('category_image_background','category_'.$slaveCategory->term_id) : get_field('category_image_background','category_'.$parentCategory->term_id);?>
								<div class="bg-category ext" style="background-image: url('<?= $img ?>')">
								...
								<div class="content">
									...
									<ul>
										...
										<li>
											...
											<script> var ean = <?= get_sub_field('prd_ean') ?>;</script>
										</li>
									...
									<?php } ?>
									<?php if(get_sub_field('prd_amazon_link') && !$widget){ ?>
										<div class="cta-amazon">
											<a href="<?= get_sub_field('prd_amazon_link') ?>" onclick="captureOutboundLink('<?= get_sub_field('prd_amazon_link') ?>', '<?= get_sub_field('prd_code') ?>', 'Amazon', '<?= getSiteSlug() ?>'); return false;" class="cta cta-orange" target="_blank"><?= get_field('buy_on_amazon_label','option') ?></a>
										</div>
									<?php } ?>
									<?php if(get_sub_field('prd_ecommerce_link') && !$widget){ ?>
										<div class="cta-amazon">
											<a href="<?= get_sub_field('prd_ecommerce_link') ?>" onclick="captureOutboundLink('<?= get_sub_field('prd_ecommerce_link') ?>', '<?= get_sub_field('prd_code') ?>', 'e-Commerce', '<?= getSiteSlug() ?>'); return false;" class="cta blue" target="_blank"><?= get_field('buy_on_ecommerce_label','option') ?></a>
										</div>
									<?php } ?>
									<?php if($widget){ ?>
										<? var_dump(get_sub_field('prd_code')) ?>
										<div class="cta-amazon">
											<div class="cswidget" data-asset-id="<?= $widget ?>" data-product-sku="<?= get_sub_field('prd_code') ?>"></div>
										</div>
								  <?php } ?>
								</div>
							</div>
	...
	<div class="container-fluid related-container">
		...
		<div class="row related_prod">
			...
			<div id="post-<?= $postId ?>" class="product-preview col-xs-12 col-sm-12 col-md-4 active">
				...
				<!--<div class="cswidget" data-asset-id="1765"></div>-->
			</div><!-- #post-## -->
			<?php endwhile; ?>
		</div>
	</div>
...

{
  ...
  "offers": {
    "@type": "Offer",
    "url": "<?= get_permalink() ?>",
    "priceCurrency": "<?= get_field('price_currency','option') ?>",
...
*/
var captureOutboundLink = function(url, code, source, slug) {
   ga('send',
	    'event',
			'btnDetail_' + code + '_' + source + '_' + slug,
			'click',
			 url, {
     'transport': 'beacon',
	...
	ga('send', {
	  'hitType': 'event',
	  'eventCategory': 'btnGeneral_' + source + '_' + slug,
	  'eventAction': 'click',
	  'eventLabel': source,
